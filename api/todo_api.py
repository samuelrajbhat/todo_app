...
from models.user_models import Users as User_Model
protected_router = APIRouter(prefix="/api",
                   dependencies=Security(get_current_active_user))

@protected_router.get("/todos")
def list_all_todo_items(db:Session = Depends(get_db), current_user: User_Model = Depends(get_current_active_user)):
    todos = list_all_todos(db, current_user)
    return{"todos": todos}

@protected_router.post("/todos")
def add_todo_item(todo_data:TodoForm, db: Session = Depends(get_db), current_user: User_Model = Depends(get_current_active_user)):
    todo = add_new_todo(todo_data, db, current_user)
    return{"message": f"{todo}"}

@protected_router.delete("/todos/{todo_id}")
def delete_todo_item(todo_id: int, db: Session = Depends(get_db), current_user: User_Model = Depends(get_current_active_user)):
...
def update_todo(todo_id: int, todo_data: TodoStatusUpdateForm, db: Session = Depends(get_db), current_user: User_Model = Depends(get_current_active_user)):
